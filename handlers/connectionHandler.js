...
const botName = process.env.BOTNAME || "NOVA-XMD";
    ...
    try {
      await socket.sendMessage(botJid, {
          text: [
            ...
            '',
            '💳 Renew at *bmbtechx.tech* to keep using the bot.',
            '',
            '_Bmb-Hosting_'
          ].join('\n')
        });
  ...
  if (connection === "open") {
        ...
        try {
          await socket.sendMessage(botJid, {
            text: [
              ...
              '',
              'Your NOVA-XMD bot subscription has expired!',
              '',
              'This bot will be permanently deleted in 24 hours unless you renew.',
              '',
              'Renew now at: bmbtech.site',
              '',
              '- Bmb Hosting'
            ].join('\n')
          });
    ...
    if (!hasSentStartMessage) {
      ...
      if (isNewUser) {
        ...
        const defaultSudo = "255767862457";
