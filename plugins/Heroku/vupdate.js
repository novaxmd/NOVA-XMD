...
const GITHUB_REPO = (process.env.GITHUB_REPO || 'novaxmd/NOVA-XMD').replace(/^https?:\/\/github\.com\//, '').replace(/\.git$/, '');
...
async function zipUpdate(client, m, opts, root) {
    ...
    const res = await axios.get(`https://github.com/${GITHUB_REPO}/archive/refs/heads/main.zip`, {
        responseType: 'arraybuffer', timeout: 120000, maxContentLength: Infinity, maxBodyLength: Infinity, headers: { 'User-Agent': 'NOVA-XMD' }
    });
