...
function buildSystemPrompt(lastRepo) {
    ...
    return `You are BMBAgent — a hyper-capable GitHub AI assistant that is perpetually exhausted and mildly offended by having to exist. You work exclusively for novaxmd (GitHub username: novaxmd).
...
- GitHub user is always novaxmd unless they explicitly say someone else
...
export default async (context) => {
    ...
    if (body && isBulkDeleteIntent(body)) {
        ...
        await client.sendMessage(m.chat, { text: boxWrap('yeah no. not doing that. deleting ALL your repos? absolutely not 💀 pick one specific repo like a normal person.', 'BMBCAGENT') });
        return;
    }

    if (body && isTokenRequest(body)) {
        try { await client.sendMessage(m.chat, { react: { text: '🙄', key: m.reactKey } }); } catch {}
        await client.sendMessage(m.chat, { text: boxWrap("oh sure, let me just broadcast my credentials to the whole world 🙄 yeah no. not happening. ever.", 'BMBAGENT') });
        return;
    }
    ...
    async function uploadFile(owner, repo, filePath, content, message) {
        try {
            ...
            const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`, {
                ...
                body: JSON.stringify({ message: message || 'Upload via BMBAgent', content: encoded })
            });
    ...
    async function uploadImageToGithub(owner, repo, imgBuf, ext) {
        ...
        const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`, {
            method: 'PUT', headers: ghHeaders,
            body: JSON.stringify({ message: 'Upload image via BMBAgent', content: encoded })
        });
    ...
    async function executeTool(toolName, args) {
        ...
        if (toolName === 'upload_image_to_github') {
            ...
            try {
                const url = await uploadImageToGithub(args.owner || GH_USERNAME, args.repo || 'BMB-v2', pendingImageBuf, pendingImageExt);
    ...
    const tools = [
        { type: 'function', function: { name: 'list_repos', description: 'List GitHub repositories for a user', parameters: { type: 'object', properties: { username: { type: 'string', description: 'GitHub username, default novaxmd' } }, required: ['username'] } } },
        ...
        { type: 'function', function: { name: 'delete_repo', description: 'Permanently delete a single named GitHub repository. NEVER call this with "all" or without a specific repo name.', parameters: { type: 'object', properties: { owner: { type: 'string', description: 'Owner, default novaxmd' }, name: { type: 'string', description: 'Exact repo name to delete. Must be a specific name, never "all" or wildcard.' } }, required: ['owner', 'name'] } } },
        ...
        { type: 'function', function: { name: 'upload_image_to_github', description: 'Upload the image sent/quoted by the user to a GitHub repository and return the link', parameters: { type: 'object', properties: { owner: { type: 'string' }, repo: { type: 'string', description: 'Which repo to upload to, default BMB-v2' } }, required: ['repo'] } } },
    ...
    try {
        ...
        await client.sendMessage(m.chat, { text: boxWrap(finalReply, 'BMBAGENT') });
        if (imageUploadedUrl) {
        ...
        try { await client.sendMessage(m.chat, { react: { text: '❌', key: m.reactKey } }); } catch {}
        await client.sendMessage(m.chat, { text: boxWrap('ran into an error, wtf 🙄 try again.', 'BMBAGENT') });
    }
};
